...
// -----------------------------------------------------------------------------
// PinMarkers
// Renders Red Spheres for:
// 1. Existing pinned comments (Saved)
// 2. Pending pin (Active double-click)
// -----------------------------------------------------------------------------
function PinMarkers() {
    const loadedModels = useStore(state => state.loadedModels);
    const comments = useStore(state => state.comments);
    const pendingPin = useStore(state => state.pendingPin);
    ...
    loadedModels.forEach(model => {
        const guids = model.elements ? model.elements.map(e => e.guid) : [];
        guids.forEach(guid => {
    ...
    // 2. Pending Pin
    if (pendingPin) {
        pins.push({ id: 'pending', pos: pendingPin, color: '#ff00ff' }); // Magenta for pending
    }

    return (
        <group>
            {pins.map(pin => (
                <mesh key={pin.id} position={[pin.pos.x, pin.pos.y, pin.pos.z]}>
...
function CameraController() {
    ...
    return <OrbitControls ref={controlsRef} makeDefault enableDamping={true} />;
}
        ...
        <div className="h-full w-full bg-[#202020] relative">
            ...
            >
                ...
                <Suspense fallback={null}>
                    {/* Visual Markers (Pinned Comments) - Rendered at Root World Space */}
                    <PinMarkers />
                ...
                {/* Controls - Disable Zoom on Double Click to allow Pinning */}
                <CameraController />
            </Canvas>
        </div>
    );
